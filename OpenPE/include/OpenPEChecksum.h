...
namespace OpenPE
{
	// Calculate Checksum of Image (perform no checks on PE Structure)
	inline uint32_t calculateChecksum(std::istream& iFileStream)
	{
			// Save Stream State
			...
			try
			{
				...
				for (long long i = 0; i < iFileSize; i += 4)
				{
				...
				// Finish Checksum
				iChecksum = (iChecksum & 0xFFFF) + (iChecksum >> 16);
